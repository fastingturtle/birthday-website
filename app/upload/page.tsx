...
// Add these component definitions after the types and before UploadPage:
...
function InstagramCard({ 
    ...
    return (
      <div>
        <div className="max-w-[468px] mx-auto bg-white rounded-lg shadow-lg">
          {/* Post Header */}
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6">
            <h2 className="text-center text-white font-bold text-2xl mb-2">
              Class of 2024
            </h2>
...
export default function UploadPage() {
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-300 to-green-400 text-white px-4 sm:px-8">
      <h1 className="text-4xl font-extrabold mb-6 text-center">Upload Your Photos</h1>

      {/* Custom Text Input */}
      <div className="mb-4 w-full max-w-lg">
        <label className="block text-white text-lg font-medium mb-2">Caption/Title:</label>
        <input
          type="text"
          value={customText}
          onChange={(e) => setCustomText(e.target.value)}
          className="w-full px-4 py-2 border rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300 text-black"
      ...
      <div
        {...getRootProps()}
        className={`flex flex-col items-center justify-center w-full max-w-lg p-6 border-2 border-dashed rounded-lg cursor-pointer ${
          isDragActive ? 'bg-blue-200 border-white' : 'hover:bg-blue-100 border-gray-200'
        }`}
      >
        <input {...getInputProps()} />
        <p>{isDragActive ? 'Drop the files here...' : 'Drag and drop files here, or click to select files'}</p>
        <p className="text-sm text-gray-200 mt-2">(Supported: Images only)</p>
      </div>

      {/* File Queue */}
      {fileQueue.length > 0 && (
        <div className="mt-6 w-full max-w-lg">
          <h2 className="text-lg font-bold mb-4">Upload Queue:</h2>
          <ul className="space-y-4">
            {fileQueue.map((item, index) => (
              ...
              >
                <span className="text-black truncate max-w-[200px]">{item.file.name}</span>
      ...
      <button
        ...
        className={`mt-6 px-6 py-3 font-bold rounded-lg shadow-md ${
          isUploading
